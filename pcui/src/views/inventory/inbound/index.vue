<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="入库单名称" prop="inboundName">
        <el-input
          v-model="queryParams.inboundName"
    ...
    <el-table v-loading="loading" :data="inboundList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="入库单名称" align="center" prop="inboundName" />
      ...
      <el-table-column label="计划入库日期" align="center" prop="plannedDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.plannedDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="实际入库日期" align="center" prop="actualDate" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-if="scope.row.approvalStatus === '0' || scope.row.approvalStatus === '1'"
          >审核</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['inventory:inbound:execute']"
          ...
          <el-button
            ...
            v-hasPermi="['inventory:inbound:query']"
          >详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="1200px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="8">
            <el-form-item label="入库单号" prop="inboundNo">
              <el-input v-model="form.inboundNo" placeholder="请输入入库单号" readonly />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="入库单名称" prop="inboundName">
              <el-input v-model="form.inboundName" placeholder="请输入入库单名称" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="入库类型" prop="inboundType">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="目标仓库" prop="warehouseId">
              <el-select v-model="form.warehouseId" placeholder="请选择目标仓库" @change="handleWarehouseChange">
                <el-option
                  v-for="warehouse in warehouseList"
                  :key="warehouse.id"
                  :label="warehouse.warehouseName"
          ...
          <el-col :span="8">
            <el-form-item label="计划入库日期" prop="plannedDate">
...
<script>
import { listInbound, getInbound, delInbound, addInbound, updateInbound, approveInbound, executeInbound, generateInboundNo } from "@/api/inventory/inbound"
...
export default {
  name: "Inbound",
  dicts: ['inbound_type', 'approval_status', 'quality_status'],
  components: {
    MaterialSelectDialog,
    ShelfSelectDialog,
    LocationSelectDialog
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getInbound(id).then(response => {
        ...
        this.detailList = response.data.inboundOrderDetailList || []
        this.open = true
        this.title = "修改入库单表"
      })
    },
    ...
    /** 审核按钮操作 */
    handleApprove(row) {
      this.$modal.confirm('是否确认审核入库单编号为"' + row.inboundNo + '"的数据项？').then(function() {
        return approveInbound(row)
      }).then(() => {
        this.getList()
    ...
    /** 执行按钮操作 */
    handleExecute(row) {
      this.$modal.confirm('是否确认执行入库单编号为"' + row.inboundNo + '"的数据项？').then(function() {
        return executeInbound(row)
      }).then(() => {
        this.getList()
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.inboundOrderDetailList = this.detailList
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除入库单表编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 生成入库单号 */
    generateInboundNo() {
      generateInboundNo().then(response => {
        this.form.inboundNo = response.data
      })
    },
    /** 仓库变化处理 */
